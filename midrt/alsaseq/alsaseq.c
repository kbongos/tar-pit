...
//------------------------------------------------------------
static char alsaseq_list__doc__[] =
"List alsa output or input ports.\n\noutflag=1 for outputs, 0 for inputs.\n"
"  returns: [(client_id, port_id, client_name, port_name),]\n";

static PyObject *
alsaseq_list(PyObject *self /* Not used */, PyObject *args)
{
 int outflag = 0;
 if (!PyArg_ParseTuple(args, "i", &outflag))
  return NULL;
  unsigned int port_flags =
            outflag ? SND_SEQ_PORT_CAP_WRITE | SND_SEQ_PORT_CAP_SUBS_WRITE
                : SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_SUBS_READ;
  ...
  while (snd_seq_query_next_client(seq_handle, client_info) == 0) {
        ...
        while (snd_seq_query_next_port(seq_handle, port_info) == 0) {
            ...
            if (((capability & port_flags) == port_flags) &&
                ...
                PyObject *pyobj = Py_BuildValue( "(i,i,s,s)", client_id, port_id, client_name, port_name );
